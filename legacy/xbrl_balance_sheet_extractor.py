...
# -*- coding: utf-8 -*-
"""
XBRL 파일에서 연결재무상태표 데이터를 추출하는 스크립트

사용법:
    python xbrl_balance_sheet_extractor.py <xbrl_file_path>

예시:
    python xbrl_balance_sheet_extractor.py 20250813001262_ifrs/entity00171636_2025-06-30.xbrl
"""
...
def extract_consolidated_balance_sheet(xbrl_file_path):
    """
    XBRL 파일에서 연결재무상태표 데이터를 추출

    Args:
        xbrl_file_path (str): XBRL 파일 경로

    Returns:
        tuple: (pd.DataFrame, dict) 연결재무상태표 데이터와 메타데이터
    """
    try:
        # XBRL 파일 읽기
        ...
        print_metadata_summary(metadata, 'BS')

        # 연결재무상태표 추출 (separate=False는 연결재무상태표)
        financial_statements = xbrl.get_financial_statement(separate=False)

        if not financial_statements:
            print("연결재무상태표를 찾을 수 없습니다.")
        ...
        if df.empty:
            print("재무상태표 데이터가 비어있습니다.")
            return pd.DataFrame(), metadata

        # 메타데이터 컬럼 추가
        df_with_metadata = add_metadata_to_dataframe(df, metadata, 'BS')
...
def save_results(df, xbrl_file_path, metadata):
    ...
    output_file = f"{base_name}_{corp_name}_{yyyy}_{month}_연결재무상태표_메타데이터포함.csv"
...
def main():
    """메인 함수"""
    if len(sys.argv) != 2:
        print("사용법: python xbrl_balance_sheet_extractor.py <xbrl_file_path>")
        print("예시: python xbrl_balance_sheet_extractor.py 20250813001262_ifrs/entity00171636_2025-06-30.xbrl")
        sys.exit(1)
    ...
    try:
        ...
        # 연결재무상태표 추출
        balance_sheet_df, metadata = extract_consolidated_balance_sheet(xbrl_file_path)

        if not balance_sheet_df.empty:
            print("\n=== 연결재무상태표 (메타데이터 포함) ===")
            print(balance_sheet_df.head(10).to_string())
            if len(balance_sheet_df) > 10:
                print(f"... 총 {len(balance_sheet_df)}행의 데이터")

            # 결과 저장
            save_results(balance_sheet_df, xbrl_file_path, metadata)

        else:
            print("연결재무상태표 데이터를 추출할 수 없습니다.")
